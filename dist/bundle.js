(function (global, factory) {
	...
	(factory((global.kiwi = {}),global.stream));
...
function peg$parse(input, options) {
  ...
  var peg$FAILED = {},
    ...
    return "";
      ...
      peg$c58 = function (directive, defname) {
    return defname.join("");
  },
    ...
    return result;
      ...
      peg$c80 = function (identifier, parameters, defname) {
    return defname.join("");
  },
...
const traverseDepthFirstSync = function (rootNode, options) {
    ...
    while (stack.length > 0) {
        ...
        for (var i = 0; i < subnodes.length; i++) stack.push([subnodes[i], subnodeData]);
    }
    return rootNode;
};
...
function all(node, selector, matches) {
	if (!matches) matches = [];
	if (selector(node)) matches.push(node);

	var nodes = subnodes_1(node);
	if (nodes) {
...
function children(node, selector, matches) {
	if (!matches) matches = [];
	var nodes = subnodes_1(node);
	if (!nodes) return matches;
	for (var i = 0; i < nodes.length; i++) {
		var child = nodes[i];
		if (selector(child)) matches.push(child);
	}
	return matches;
}
...
const build = function (node) {
	tree(node, {
		subnodesAccessor: function (node) {
			...
			if (!list) return [];
			for (var i = 0; i < list.length; i++) list[i].parent = node;
			return list;
		}
	});
	return node;
...
const getUniformsAndAttributes = function (ast) {
	const uniforms = query.all(ast, query.selector('declarator[typeAttribute] > type[qualifier=uniform]'));
};
